...
for i in range(2, 7):
    ...
    ax[0].imshow(img_i)
    ax[0].set_title(f"Originalna slika")
    ax[0].axis('off')
    
    ax[1].imshow(img_i_kvant)
    ax[1].set_title(f"Kvantizirana slika")
    ax[1].axis('off')
    
    ax[2].plot(list(K_range), inertias, 'o-')
    ax[2].set_title('Lakat metoda')
    ax[2].set_xlabel('Broj grupa K')
    ax[2].set_ylabel('Kriterijska funkcija J')
