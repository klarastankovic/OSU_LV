...
y_test = y_test.ravel()
classes_train, train_counts = np.unique(y_train, return_counts=True)
classes_test, test_counts = np.unique(y_test, return_counts=True)

x_pos = np.arange(len(classes_train))
width = 0.35
...
# c) parametri modela - multinomijalna logistička regresija
print('\nModel parameters:')
...
y_train_ext = y_train_ext.ravel()
y_test_ext = y_test_ext.ravel()

model_ext = LogisticRegression(max_iter=5000)
model_ext.fit(X_train_ext, y_train_ext)
 
y_test_ext_p = model_ext.predict(X_test_ext)

cm = confusion_matrix(y_test_ext, y_test_ext_p)
disp = ConfusionMatrixDisplay(cm)
disp.plot()

print("Classification report (extended model):")
print(classification_report(y_test, y_test_ext_p,
                             target_names=[labels[i] for i in range(3)]))
